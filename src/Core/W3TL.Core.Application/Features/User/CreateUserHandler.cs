...
public class CreateUserHandler(IUserRepository userRepository, IUnitOfWork unitOfWork) : ICommandHandler<CreateUserCommand> {
    public async Task<Result> HandleAsync(CreateUserCommand command) {
        ...
        // Create user
        var user = global::User.Create(command.UserId, command.UserName, command.FirstName, command.LastName, command.Email, command.Pub);
